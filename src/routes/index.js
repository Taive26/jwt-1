...
router.post('/login', (req, res) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (!user) {
      res.redirect(`/login?message=${encodeURIComponent('Wrong username!')}&username=${encodeURIComponent(username)}`);
    }
  ...
  res.redirect(`/login?message=${encodeURIComponent('Oops! Something went wrong!')}&username=${encodeURIComponent(username)}`);
});
